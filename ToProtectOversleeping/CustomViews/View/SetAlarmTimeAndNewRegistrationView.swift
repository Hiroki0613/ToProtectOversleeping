...
class SetAlarmTimeAndNewRegistrationView: UIView, UITableViewDelegate {
    ...
    var chatTeamNewRegisterButton = WUButton(backgroundColor: .clear, title: "新規登録")
    var chatTeamNewInvitedButton = WUButton(backgroundColor: .clear, title: "招待される")
    var chatTeamNameAndRegstrationStackView = UIStackView(frame: .zero)
    
    // 戻るボタン
    let wakeUpGoBuckButton = WUButton(backgroundColor: .clear, title: "戻る")
